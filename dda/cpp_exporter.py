...
import textwrap, itertools, math, functools
...
def run(command="./a.out", return_ndarray=True, arguments={}, fields_to_export=[], binary=False):
    """
    ...
    of data (since CSV generating and parsing overload is gone). This option only makes real sense if
    you set ``return_ndarray=True``. Note that if you don't pass the  ``fields_to_export`` option
    but set ``binary=True``, in the moment the returned array is one-dimensional.
    ...
    # FIXME: Python 3.6 does not yet have the tex keyword. Newer Pythons have. Find something
    #        compatible.
    ...
    if proc.returncode:
        ...
        if return_ndarray:
            ...
            if binary:
                ...
                data = fromfile(datafh, dtype="<f8")
                if fields_to_export:
                    N = len(data) / len(fields_to_export)
                    data = data.reshape(N, len(fields_to_export))
                else:
                    # Should warn the user about data size mismatch
                    pass
            else:
